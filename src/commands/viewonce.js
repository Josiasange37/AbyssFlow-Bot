module.exports = {
  ...
  async execute({ sock, chatId, message, bot }) {
    try {
      ...
      if (!quoted) {
        return await bot.sendSafeMessage(chatId, `faut répondre à une vue unique avec la commande .`);
      }
      ...
      if (!viewOnce) {
        return await bot.sendSafeMessage(chatId, `c'est pas une vue unique ça .`);
      }
